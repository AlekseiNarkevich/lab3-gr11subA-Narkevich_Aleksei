...
**Вариант:** 1 (нечетный номер в списке группы)
...
Expected: 4000000
Actual: 1165089
Time: 0.100517 seconds
```

**Комментарий:**  
При запуске в режиме без синхронизации наблюдается серьезная гонка данных (race condition). Ожидаемое значение 4,000,000 (4 потока × 1,000,000 инкрементов), но фактическое значение всего 1,165,089. Это происходит потому, что несколько потоков одновременно читают и записывают в одну и ту же ячейку памяти, перезаписывая результаты друг друга. Выполнение программы занимает 0.100517 секунд.
...
Expected: 4000000
Actual: 4000000
Time: 0.244575 seconds
```

**Комментарий:**  
При использовании мьютекса результат корректный — 4,000,000. Однако время выполнения увеличилось почти в 2.5 раза (с 0.100517 до 0.244575 секунд) из-за накладных расходов на блокировку и разблокировку мьютекса для каждого инкремента.
...
Expected: 4000000
Actual: 4000000
Time: 0.161959 seconds
```

**Комментарий:**  
Использование атомарных операций также обеспечивает корректность результата (4,000,000), но время выполнения меньше, чем при использовании мьютекса (0.161959 против 0.244575 секунд). Это связано с тем, что атомарные операции обычно реализуются на уровне процессора и не требуют системных вызовов для блокировок.

#### Сравнение производительности
| Режим       | Ожидаемое значение | Фактическое значение | Время (сек) | Корректность |
|-------------|-------------------|---------------------|-------------|--------------|
| unsync      | 4000000           | 1165089             | 0.100517    | Нет          |
| mutex       | 4000000           | 4000000             | 0.244575    | Да           |
| atomic      | 4000000           | 4000000             | 0.161959    | Да           |
...
**Комментарий:**  
Программа корректно обработала все 100,000 элементов. Количество произведенных и потребленных элементов совпадает, а также совпадает сумма их значений. Это подтверждает корректность синхронизации с использованием мьютексов и условных переменных. Время выполнения составило 0.0553 секунды.
...
**Комментарий:**  
При использовании одного производителя и одного потребителя программа работает очень быстро (0.0021 секунды) и корректно обрабатывает все элементы.

#### Запуск с 2 производителями и 2 потребителями (10000 элементов)
```bash
make run_custom ARGS="-P 2 -C 2 -N 10000 -B 64"
```
...
**Комментарий:**  
При увеличении количества потоков с 1 до 2 время выполнения выросло с 0.0021 до 0.0058 секунд (в 2.76 раза). Это связано с накладными расходами на синхронизацию.

#### Запуск с 4 производителями и 4 потребителями (10000 элементов)
```bash
make run_custom ARGS="-P 4 -C 4 -N 10000 -B 64"
```
...
**Комментарий:**  
При увеличении количества потоков с 2 до 4 время выполнения выросло с 0.0058 до 0.0115 секунд (в 1.98 раза). Несмотря на увеличение конкуренции за доступ к буферу, программа продолжает работать корректно.

#### Сравнение производительности
| Параметры | Количество элементов | Время (сек) | Корректность |
|-----------|---------------------|-------------|--------------|
| P=1, C=1  | 10000               | 0.0021      | CORRECT      |
| P=2, C=2  | 10000               | 0.0058      | CORRECT      |
| P=4, C=4  | 10000               | 0.0115      | CORRECT      |
| P=2, C=2  | 100000              | 0.0553      | CORRECT      |

**Вывод:**  
С увеличением количества потоков время выполнения растет нелинейно из-за накладных расходов на синхронизацию. Однако программа сохраняет корректность работы благодаря правильному использованию примитивов синхронизации.

---

## Задание C: Где видны потоки

### Скрипт для анализа потоков (`threads_info.sh`)
```bash
#!/bin/bash
...
PID of long-running process (sleep): 15189
Process is running. Analyzing threads...
=== Threads in ps ===
    PID     TID PSR %CPU STAT COMMAND
  15189   15189   0  0.0 S+   sleep
...
=== Threads in /proc/<PID>/task ===
итого 0
dr-xr-xr-x 7 alexfresh-1 alexfresh-1 0 ноя 12 11:29 15189
...
1. **`ps -L`**: Показывает один поток (TID=15189) с PID=15189. Состояние процесса — `S+` (спящий в интерактивном режиме), использование CPU — 0%.

2. **`/proc/<PID>/status`**: Показывает, что процесс имеет 1 поток (`Threads: 1`).

3. **`/proc/<PID>/task`**: Показывает один подкаталог (15189) в директории задач, что соответствует одному потоку.
...
**Пример из практики:**
В задании A, если бы мы использовали массив счетчиков по одному на поток, и эти счетчики находились рядом в памяти, это могло бы привести к ложному совместному использованию. Чтобы этого избежать, можно использовать выравнивание или локальные счетчики с последующим суммированием.
